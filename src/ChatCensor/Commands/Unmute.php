    		...
    		if(isset($args[0])){
    			...
    			if($this->plugin->getServer()->getPlayer($args[0]) != null){
    				...
    				if($this->plugin->isMuted($player)){
    				    ...
    				    $sender->sendMessage(TextFormat::colorize(ChatCensor::PREFIX . " &aYou unmuted &b" . $player));
    					if($this->plugin->cfg["mute"]["log-unmute"]){
    					    $this->plugin->getServer()->getPlayer($player)->sendMessage(TextFormat::colorize($this->plugin->replaceVars($this->plugin->getMessage("unmuted"), array("PREFIX" => ChatCensor::PREFIX, "PLAYER" => $sender->getName()))));
    					}
    				}else{
    				    $sender->sendMessage(TextFormat::colorize(ChatCensor::PREFIX . " &cPlayer " . $player . " is not muted!"));
    				}
    			}else{
    			    $sender->sendMessage(TextFormat::colorize(ChatCensor::PREFIX . " &cPlayer not found!"));
    			}
    		}else{
    		    $sender->sendMessage(TextFormat::colorize(ChatCensor::PREFIX . " &cUsage: /unmute <player>"));
    		}
    	}else{
